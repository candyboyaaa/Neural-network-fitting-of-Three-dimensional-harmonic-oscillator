...
def energy_loss(params, model, r, key):
    ...
    # 对于l=1, m=0的球谐函数，径向哈密顿量
    # 动能项：-(hbar²)/(2μ) * [∂²ψ/∂r² + (2/r) * ∂ψ/∂r - l(l+1)/r² * ψ]
    # 其中l=1，所以l(l+1)=2
    kinetic = - (hbar**2) / (2 * mu) * (d2psi_dr2 + 2/r_safe * dpsi_dr - 2.0 / (r_safe**2) * psi)
    potential = V * psi
    energy_density = psi * (kinetic + potential)
    
    # 蒙特卡洛积分（均匀采样）
    r_max = 8.0  # 积分上限
    # 神经网络拟合的是径向函数R(r)，哈密顿量是径向哈密顿量
    # 对于径向函数，正确的积分权重是r²
    integrand = energy_density * r.squeeze()**2
    total_energy = jnp.mean(integrand) * r_max  # 乘以积分区间长度
    
    # 计算归一化因子 - 径向函数的归一化
    # 对于径向函数R(r)，正确的归一化是∫ |R(r)|² r² dr = 1
    norm_integrand = (psi**2) * r.squeeze()**2
...
def normalize(params, model, key, num_samples=10000, r_max=8.0):
    ...
    # 径向函数的归一化
    # 对于径向函数R(r)，正确的归一化是∫ |R(r)|² r² dr = 1
    norm = jnp.sqrt(jnp.mean((psi**2) * r.squeeze()**2) * r_max)
    return norm
...
def train():
    ...
    plt.xlabel('steps')
    plt.ylabel('energy')
    plt.title('Energy for neural network fitting of the first excited state of a three-dimensional harmonic oscillator in spherical coordinates')
        ...
        for i in range(resolution):
            for j in range(resolution):
                for k in range(resolution):
                    ...
                    if r <= 5.0 and r > 1e-8:  # 只计算在积分范围内的点，避免r=0
                        # 径向波函数部分
                        psi_radial = model.apply(params, np.array([[r]]))
                        # l=1, m=0的球谐函数：Y₁₀(θ,φ) = √(3/4π) * cos(θ) = √(3/4π) * z/r
                        Y_10 = np.sqrt(3.0 / (4.0 * np.pi)) * Z[i,j,k] / r
                        # 完整的波函数 = 径向部分 × 球谐函数
                        psi_values[i,j,k] = float(psi_radial.squeeze()) * Y_10 / norm
                    else:
                        psi_values[i,j,k] = 0.0  # 在r=0处波函数为0（l=1的要求）
    ...
    # 保存为DS3文件格式
    def save_ds3_file(X, Y, Z, psi_values, filename='Neural network fitting of the first excited state of a three-dimensional harmonic oscillator in spherical coordinates.ds3'):
        with open(filename, 'w') as f:
            # 写入文件头
            f.write("# DS3文件格式 - 三维谐振子l=1,m=0波函数\n")
    ...
    print(f"波函数已保存为 Neural network fitting of the first excited state of a three-dimensional harmonic oscillator in spherical coordinates.ds3")
